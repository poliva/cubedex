import{p as I}from"./chunk-6APJHPXM-DWt5x9_q.js";import{H as N,w as E,S as y}from"./index-D-8tbuyU.js";var J=null;async function pB(){return J??(J=K())}async function K(){return I(await E.fto.kpuzzle(),`SubgroupSizes 24 12 11 12 12 11 10 9 11 8 10 9 7 8 10 10 6 9 8 5 7 6 5 4 4 8 7 3 6 6 360
...
`)}function h(R){if(R<2)return R;let B=1;for(let L=2;L<=R;L++)B*=L;return B}function m(R,B){if(B<0||B>R)return 0;if(B===0||B===R)return 1;let L=1;for(let F=0;F<B;F++)L=L*(R-F)/(F+1)|0;return L}function P(R){R=R.slice();let B=R.length,L=h(B-1),F=0;for(;B>1;){B--;let l=R[0];F+=l*L;for(let U=0;U<B;U++){let g=R[U+1];R[U]=g-(g>l)}L/=B}return F}function Q(R,B){let L=[],F=h(B-1);for(let l=0;l<B;l++)L[l]=R/F|0,R%=F,F/=B-1-l;for(let l=B-2;l>=0;l--)for(let U=l+1;U<B;U++)L[U]+=+(L[U]>=L[l]);return L}function b(R){let B=R.length,L=0;for(let F=0;F<B-1;F++)for(let l=F;l<B;l++)R[F]>R[l]&&(L^=1);return L}function V(R,B){let L=[],F=h(B-1)/2,l=0;for(let U=0;U<B-1;U++)L[U]=R/F|0,R%=F,F/=B-1-U;L[B-1]=0;for(let U=B-2;U>=0;U--)for(let g=U+1;g<B;g++)L[g]>=L[U]?L[g]++:l^=1;return l===1&&([L[B-2],L[B-1]]=[L[B-1],L[B-2]]),L}function x(R){return P(R)>>1}function T(R){let B=R.length,L=0;for(let g=0;g<B;g++)L+=+(R[g]===1);let F=B-L;if(F===0||L===0||B===1)return 0;let l=m(B-1,L),U=0;for(let g=0;F>0&&L>0&&B>1;g++)B--,R[g]===0?l=l*--F/B:(U+=l,l=l*L--/B);return U}function W(R,B,L){let F=L-B,l=m(L-1,B),U=[],g=L-1;for(let A=0;A<g;A++)L--,R<l?(U.push(0),l=l*--F/L):(U.push(1),R-=l,l=l*B--/L);return U.push(B),U}function p(R,B){let L=[];for(let F=0;F<B.length;F++)L[F]=R[B[F]];return L}function i(R,B){let L=[];for(let F=0;F<B;F++)L[F]=F;for(let F=0;F<R.length;F++)L[R[F]]=R[(F+1)%R.length];return L}function f(R,B){let L=Array(B).fill(0);for(let F in R)L[F]=R[F];return L}function X(R,B){let L=Array(8).fill(0);for(let F=0;F<8;F++)L[F]=(R[1][F]+B[1][F])%3;return[p(R[0],B[0]),L]}var Y=[i([0,1,4],12),f({0:2},8)],Z=[i([1,2,5],12),f({1:2},8)],$=[i([2,3,6],12),f({2:2},8)],BB=[i([3,0,7],12),f({3:2},8)],LB=[i([9,8,4],12),f({4:2},8)],RB=[i([10,9,5],12),f({5:2},8)],FB=[i([11,10,6],12),f({6:2},8)],UB=[i([8,11,7],12),f({7:2},8)];Q(0,12),Array(8).fill(0);var s=[Y,Z,$,BB,LB,RB,FB,UB],lB=["UL","U","UR","F","L","B","R","D"],_=[0,1,0,1,1,0,1,0];function DB(R){let B=["0","","'"];return R.map(([F,l])=>lB[F]+B[l]).join(" ")}function gB(){let R=V(Math.floor(y(h(12))/2),12),B=Array(8);for(let L=0;L<8;L++)B[L]=y(3);return[R,B]}function AB(){return eB(gB())}function eB(R){let B=tB(R),L=[S(),j()],F=[nB(),oB()],l=[z(),aB()],U=[iB(),rB()],g=uB(B,L,F),A,D=new Set,r=+new Date;for(let o=0;o<25;o++){let{value:t,done:n}=g.next(),a=R;for(let[u,H]of t)for(let w=0;w<H;w++)a=X(a,s[u]);if(D.has(a.toString()))continue;D.add(a.toString());let O=x(a[0].slice(0,8)),d=0;for(let u=0;u<4;u++)d+=a[1][u]*3**u;let q=[O,d],G=A?A.length-t.length-1:999999,c=fB(q,l,U,G);if(c!==void 0&&((A===void 0||A.length>t.length+c.length)&&(A=t.concat(c)),new Date-r>300))break}return A}function tB(R){let B=0;for(let U=0;U<4;U++)B+=R[0].indexOf(U+8)*12**U;let L=0;for(let U=0;U<4;U++)L+=R[1][U+4]*3**U;let F=R[0].map(U=>Math.max(-1,U-8)),l=T(F.map(U=>+(U>=0)))*2+b(F.filter(U=>U>=0));return[B,L+81*l]}var e={};function M(){if(e.phase1cm)return e.phase1cm;let R=[];for(let B=0;B<81;B++){R[B]=Array(8);let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B][0]=R[B][1]=R[B][2]=R[B][3]=B;for(let F=0;F<4;F++)L[F]=(L[F]+2)%3,R[B][4+F]=L[0]+L[1]*3+L[2]*9+L[3]*27,L[F]=(L[F]+1)%3}return e.phase1cm=R}function rB(){if(e.phase1cp)return e.phase1cp;let R=Array(81);for(let B=0;B<81;B++){let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B]=(L[0]!==0)+(L[1]!==0)+(L[2]!==0)+(L[3]!==0)}return e.phase1cp=R}function S(){if(e.phase1em)return e.phase1em;let R=[];for(let L=0;L<12;L++){R[L]=[];for(let F=0;F<8;F++)R[L][F]=s[F][0].indexOf(L)}let B=Array(12**4);for(let L=0;L<12**4;L++){B[L]=Array(8);for(let F=0;F<8;F++){let l=0;for(let U=0;U<4;U++)l+=R[Math.floor(L/12**U)%12][F]*12**U;B[L][F]=l}}return e.phase1em=B}function nB(){return e.phase1ep?e.phase1ep:e.phase1ep=v(S(),[8+12*(9+12*(10+12*11))])}function j(){if(e.phase1sm)return e.phase1sm;const R=m(12,4);let B=[];for(let l=0;l<R;l++){B[l]=[];let U=W(l,4,12),g=[];for(let A=0,D=0;A<12;A++)U[A]===0?g[A]=-1:g[A]=D++;for(let A=0;A<8;A++){let D=p(g,s[A][0]),r=p(U,s[A][0]),o=b(D.filter(t=>t>=0));B[l][A]=T(r)*2+o}}let L=M(),F=[];for(let l=0;l<R;l++)for(let U=0;U<81;U++){let g=F[U+81*(2*l)]=[],A=F[U+81*(2*l+1)]=[];for(let D=0;D<8;D++)g[D]=L[U][D]+81*B[l][D],A[D]=L[U][D]+81*(B[l][D]^1)}return e.phase1sm=F}function oB(){return e.phase1sp?e.phase1sp:e.phase1sp=v(j(),[0])}function aB(){if(e.phase2cm)return e.phase2cm;let R=M(),B=Array(81);for(let L=0;L<81;L++)B[L]=R[L].slice(4,8);return e.phase2cm=B}function z(){if(e.phase2em)return e.phase2em;const R=8,B=h(R)/2;let L=Array(B),F=[0,1,2,3,4,5,6,7];for(let l=0;l<B;l++){L[l]=Array(4);for(let g=0;g<4;g++){let A=p(F,s[g][0].slice(0,8));L[l][g]=x(A)}if(l===B-1)break;let U=0;do for(let g=R-2;g>=0;g--){if(F[g]>F[g+1])continue;let A=g+1;for(let D=A;D<R;D++)F[D]>F[g]&&(A=D);[F[g],F[A]]=[F[A],F[g]],U^=1;for(let D=0;g+1+D<R-1-D;D++,U^=1)[F[g+1+D],F[R-1-D]]=[F[R-1-D],F[g+1+D]];break}while(U!==0)}return e.phase2em=L}function iB(){return e.phase2ep?e.phase2ep:e.phase2ep=v(z(),[0])}function v(R,B){let L=R.length,F=R[0].length,l=Array(L).fill(-1),U=B.slice(),g=[],A=0;for(;U.length>0;){g.length=0;for(let D of U)if(l[D]===-1){l[D]=A;for(let r=0;r<F;r++){let o=R[D][r];for(;o!==D;)g.push(o),o=R[o][r]}}[U,g]=[g,U],A+=1}return l}function fB(R,B,L,F){F=F||999999;let l=R.length,U=0;for(let g=0;g<l;g++)U=Math.max(U,L[g][R[g]]);for(;U<=F;){let g=C(R,B,L,U,-1);if(g!==void 0)return g;U++}}function C(R,B,L,F,l){let U=R.length,g=B[0][0].length,A=0;for(let D=0;D<U;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0)return[];if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===l||D<l&&_[D]===_[l])continue;let r=R.slice();for(let t=0;t<U;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=C(r,B,L,F-1,D);if(t!==void 0)return[[D,o]].concat(t);for(let n=0;n<U;n++)r[n]=B[n][r[n]][D];o++}}}}function*uB(R,B,L){let F=R.length,l=0;for(let U=0;U<F;U++)l=Math.max(l,L[U][R[U]]);for(;;)yield*k(R,B,L,l,-1),l++}function*k(R,B,L,F,l){let U=R.length,g=B[0][0].length,A=0;for(let D=0;D<U;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0){yield[];return}if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===l||D<l&&_[D]===_[l])continue;let r=R.slice();for(let t=0;t<U;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=k(r,B,L,F-1,D);for(;;){let{value:n,done:a}=t.next();if(a)break;yield[[D,o]].concat(n)}for(let n=0;n<U;n++)r[n]=B[n][r[n]][D];o++}}}}async function _B(){return new N(DB(AB()))}export{_B as getRandomRediCubeScramble,pB as sgsDataFTO};
